...
from model import DRCNet_Abla  # 在这里调整模型
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (
                input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, img_name,
                support_image, new_shape) in enumerate(
            val_loader):
            ...
            #                                    cat_idx=subcls, query_name=img_name[0], support_name=support_image[0],
            #                                    new_shape=new_shape)
            output, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                               cat_idx=subcls)
