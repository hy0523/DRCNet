...
def main():
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     ...
                                     transform_tri=train_transform_tri, mode='train', \
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        # input = [data.cuda(non_blocking=True) for data in input]
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        target_b = target_b.cuda(non_blocking=True)
        # s_input = [data.cuda(non_blocking=True) for data in s_input]
