...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_vgg_base.yaml',
...
def main():
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.BaseData(split=args.split, mode='train', data_root=args.data_root,
                                      data_list=args.train_list, \
                                      data_set=args.data_set, use_split_coco=args.use_split_coco, \
                                      transform=train_transform, main_process=main_process(),
    ...
    if args.evaluate:
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.BaseData(split=args.split, mode='val', data_root=args.data_root, data_list=args.val_list, \
                                        data_set=args.data_set, use_split_coco=args.use_split_coco, \
                                        transform=val_transform, main_process=main_process(),
