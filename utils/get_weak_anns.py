...
def find_bbox(mask):
    _, labels, stats, centroids = cv2.connectedComponentsWithStats(mask.astype(np.uint8)) # [x, y, w, h]
...
if __name__ == '__main__':
    label_path = '2007_001377.png'
    mask = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
    bboxs = find_bbox(mask) # (x, y, w, h)
    mask_color = cv2.imread(label_path, cv2.IMREAD_COLOR)
    from IPython import embed
    embed()
    for j in bboxs:
        cv2.rectangle(mask_color, (j[0], j[1]), (j[0] + j[2], j[1] + j[3]), (255, 255, 255), -1)
